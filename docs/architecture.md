...
Settings reach the app through three layers, lowest precedence first:

1. **Build** — `__GEOLIBRE_BUILD_ENV__`, injected by `vite.config.ts` from the `BUILD_ENV_KEYS` allowlist (bare names such as `CESIUM_TOKEN` are bridged to their `VITE_` form there).
2. **Deployment** — `window.__GEOLIBRE_DEPLOYMENT_ENV__`, written by `docker/entrypoint.sh` into `geolibre-runtime-config.js` on every container start from a fixed table of `GEOLIBRE_<NAME>` variables, each validated for shape before it is published as `VITE_<NAME>`. Share, auth and assistant settings read this object directly (`readDeploymentEnvValue`); everything else sees it because `useRuntimeEnvironmentVariables` folds every `VITE_*` value into the runtime layer just above the build.
3. **Runtime** — `window.__GEOLIBRE_RUNTIME_ENV__`, composed by `useRuntimeEnvironmentVariables` from the layers below plus the device's AI keys, the project's geocoding preference, the device Cesium token and the project's free-form Environment Variables, and re-published on `geolibre:runtime-env-change`.

`getRuntimeEnvironment()` (`@geolibre/core`) returns build + runtime, so an accessor built on it — `getCesiumIonToken()`, `getFeedbackTarget()`, `getWhereAmIConfig()`, `getCoordsConverterUrl()`, `getBranding()`, `deploymentCatalogUrls()` — honours all three layers; `getBuildEnvironment()` deliberately ignores the upper two for settings a project file must not be able to relax (the auth gate, the deployment profile, embed origins). Two deployment settings act on the _project_ rather than the env: the deployment's geocoder becomes the default of a project that has not chosen one (`useDeploymentGeocoderDefault`), and `START_PROJECT_URL` is the project a launch opens when the page URL names none (`startupProjectUrl()`).
