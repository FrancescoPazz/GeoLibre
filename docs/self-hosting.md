...
| `GEOLIBRE_CESIUM_TOKEN`, `GEOLIBRE_CESIUM_TERRAIN_ASSET_ID`, `GEOLIBRE_ELEVATION_MEAN_SEA_LEVEL`, `GEOLIBRE_GEOCODER_PROVIDER` / `_API_KEY` / `_ENDPOINT` / `_REVERSE_ENDPOINT` / `_EMAIL`, `GEOLIBRE_FEEDBACK_URL` / `_SUBJECT`, `GEOLIBRE_WHERE_AM_I_URL` / `_ACCURATE_URL` / `_ID_FIELD` / `_FIELD` / `_DETAIL_FIELD`, `GEOLIBRE_COORDS_CONVERTER_URL`, `GEOLIBRE_BRAND_NAME` / `_LOGO_URL` / `_LOGO_LINK` / `_FAVICON_URL` / `_ACCENT_COLOR` | as needed                                         | Application settings the prebuilt image takes at container start, each published to the app as the matching `VITE_` variable described in [Getting Started](getting-started.md): the 3D globe's Ion token and terrain asset, sea-level heights, the default geocoder (a deployment's own address service becomes the project default), the feedback channel, the place name under the pointer, the coordinate converter and the branding. Values that can never work — a colour that is not `#rrggbb`, a service that is not an http(s) URL — fail the boot with a message. The Ion token and the geocoder key are sent to every browser: they are the deployment's public client credentials, not secrets.                                                                                                                                                                                                                                                                                                                                                                                          |
