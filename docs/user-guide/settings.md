...
| **Reopen the last project**              | Open the most recently used _local_ project.                                                                        |
...
- **Only local projects are reopened.** Opening a share link records it in your recent projects by its `https://` URL, so _Reopen the last project_ skips remote entries rather than fetching a third-party host on every launch.
