...
- MapLibre map workspace
  ...
  - **Catalog** (Catalog plugin): a browsable tree of a geoportal's data services read from one or more TerriaJS init files — groups, WMS, WMTS and ArcGIS map services (with map-service groups expanded into their sublayers on demand) — where clicking an entry adds it as an ordinary layer and clicking again removes it; the file's initial workbench opens on an empty project and its home view is one click away. Catalog URLs come from the deployment (`CATALOG_URLS`, which also opens the panel by default), from the panel, or from the project file. Entries can be restricted to user groups (`allowedGroups`, hidden or shown locked), asked with the signed-in user's header (`useAuthentication`), given a popup from their `featureInfoTemplate` (with per-profile fields), a search field, clustering, a no-export flag and per-value colours; `3d-tiles` items add a Cesium Ion tileset and `rer-poi` items the regional points of interest
  - **Sign in** (toolbar, when the deployment names a `LOGIN_SERVICE_URL`): HTTP Basic against the geoportal's map server, session kept in memory only; unlocks restricted catalog entries and carries the header to protected services
  - **Regional points of interest** (`rer-poi` catalog items): the region's POI service loaded for the viewport — the levels the zoom asks for, a padded extent so panning does not refetch, paged requests, labels once few enough points are shown, clustering, one colour per category
  - **Regional basemaps**: a collapsed Regional section in both the New Project and Change Basemap panels for providers with local coverage the global defaults lack, starting with five keyless Chinese basemaps (高德地图, 高德卫星, 高德混合, 腾讯地图, 腾讯深色), with the same providers plus Tianditu available through the Basemaps control plugin; and an Emilia-Romagna group with the region's topographic database (DBTR webmap, DBTR CTR multiscala) and orthophoto campaigns (AGEA 2020, CGR 2018, AGEA 2011) from its cached ArcGIS services, each over a Positron base so the map does not go blank at the region's border, plus the DBTR at 70% over the 2018 orthophoto
  ...
  - **3D measuring on the globe** (Controls → 3D Measure): draw a line, polygon, points, an angle or a circle straight on the terrain — click to add vertices, click a segment to insert one, drag a vertex to move it, click the first vertex to close a polygon — with geodesic segment lengths, perimeter, geodetic area, angle and circle radius/area labelled on the map and listed in the panel, draped-on-terrain and labels toggles, and the figure kept in the project file; a line or polygon is also walked on the terrain at an adaptive 1-2-5 sampling step (auto from the path length and zoom, or picked by hand) to give the ground and air distances, the elevation range and an elevation profile chart whose hover marks the spot on the globe; heights can be referred to mean sea level with the bundled EGM96 geoid (a per-tool toggle, with a deployment default); **Save as layer** turns the measurement into a GeoJSON project layer (the figure, its vertices with per-segment distances, and the profile line with heights) that the Layers panel exports to any format, and **Summary** downloads the plain-text `name: value` report
  - **Play Path** (Controls → Play Path): fly the camera along the path drawn with 3D Measure, following the terrain — resampled at a zoom-aware step or one picked by hand, looking along the path from the distance and tilt the view had when play was pressed, waiting for terrain tiles at each stop; a short countdown, play/pause/resume, stop-and-return, automatic direction from whichever end is nearer, and a hint when the view is too flat
  - **Line of sight on the 3D globe** (Controls → Line of Sight, the first of the 3D Tools): click an observer and a target and the sight line is tested against the globe's loaded terrain — the clear stretch drawn green, the hidden remainder red, with the total and clear distances and adjustable observer and target heights. Re-tested as finer terrain tiles load, kept in the project file, and the panel explains when to switch to the globe if the 2D map is primary
  - **Globe clipping** (Controls → Globe clipping): cut a square hole in the terrain around a 3D Tiles or GeoJSON layer — sized from the loaded tileset's bounding sphere or the features' extent — so tunnels, boreholes and underground models can be seen from above; one layer at a time, moved by picking another, waited for when the tileset has not loaded its root yet, filled back in on close, with the choice kept in the project file
  - **Elevation bands** (Controls → Elevation bands): colour the globe's terrain between chosen heights with up to twelve translucent colour bands at an adjustable opacity; heights can be entered above mean sea level (shifted by the EGM96 undulation at the view centre), and the colours come off again with one click
  - **Coordinate converter** (Controls → Coordinate converter, when the deployment names a conversion service): type a position as latitude, longitude or as x, y — or drop a pin and drag it — pick the pair of systems (WGS84 to and from Monte Mario / Gauss-Boaga, ED50, ETRS89, RDN2008 and UTM) and read the result to 6 decimals in degrees or 4 in metres; copy either field or centre the map on it. The datum shifts run on the server's NTv2 grids
  - **Query data** (Controls → Query data, when a catalog layer declares `queryableProperties`): one filter control per declared property — a pick-list for an enumeration, a range for a number, a from/to for a date — kept as the layer's quick filters so the map narrows with the query; the matching features aggregated by a groupable property as a count or as the sum of a summable one (spread over several categories through a dictionary property, or over the values of a multi-value one), drawn as a pie, bars or a pivot table; the matching features as a sortable table; chart (PNG), pivot table and table (CSV) and the features themselves (GeoJSON + CSV) downloadable. Gated by the signed-in profile's `QueryData` / `DownloadQueryData` allowances when the deployment defines profiles
